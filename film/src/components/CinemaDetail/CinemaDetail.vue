<template>
    <div id="cinema-detail">
      ...
      <div class="cinema-info">
        <span class="name">糖豆影院</span>
        <span class="address"><span class="icon icon-address"></span>金水区566号糖豆广场</span>
        <span class="tel"><span class="icon icon-tel"></span>15212345678</span>
      </div>
      <el-carousel
        :autoplay=false
        type="card"
      ...
      <!-- <div class="bei">
        ...
        <el-carousel-item v-for="(item,index) in hasMovieInfo" :key="index">
          <a href="#" @click.prevent="$router.push({path:'/movie+datail',query:{movie_id:item.movie_id}})"><img :src="server+item.poster" alt=""></a>
        </el-carousel-item>
      </el-carousel>
      <div class="movie-info" v-for="(item,index) in hasMovieInfo" :key="index" v-show="movieIndex===Number(index)">
        <span class="arrow"></span>
        <span class="main"><span class="name">哪吒 魔童降世</span><span class="score"><span class="num" v-if="item.score">{{item.score.toFixed(1)}}分</span><span v-else style="font-size: .28rem">暂无评分</span></span></span>
        <span class="intro">
        <span class="time">110分钟</span><span class="split">|</span><span class="type">喜剧/动画</span><span class="split">|</span><span class="actors">饺子</span>
        </span>
      </div>
      <ly-tab
        v-model="selectedId"
      ...
      <div class="ticket-container">
        <div class="item" v-for="(item,index) in movieDaySchedule" :key="index">
          <div class="left">
            <span class="start-date">{{item.show_item}}</span>
            <span class="end-date">13:55散场</span>
          </div>
          <div class="center">
            <div class="info">
          ...
          <div class="right">
            <span class="buy-btn" @click="$router.push({path:'/select_seat',query:{cinema_id:item.cinema_id,movie_id:item.movie_id,schedle_id:item.schedle_id}})">购票</span>
          </div>
        </div>
      </div>
    </div>
